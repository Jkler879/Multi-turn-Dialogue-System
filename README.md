...
**ReAct Agent** + **Rag** 开发的**生产级多轮对话系统**，包含以下核心功能：
  ...
  - **自主规划思考**:  通过 `LanGraph状态图编排`规划多个工具集的自主调用，包括`混合检索、实体关系验证、翻译等工具`。

  - **长短期记忆提取**:  通过`多级记忆（短期+长期）+ 查询改写(指代消解)`，在多轮对话中保持`上下文连贯`、`指代准确`、`持久化存储用户画像`。

  - **中英双语交互**:  支持 `中文问-英文检-中文回复` 的完整链路，保证知识库为英文数据集的同时，支撑`用户中文输入与系统中文回复`。
...
- **短期记忆**：`Redis Hash` + `Sorted Set` 存储最近 5 轮对话，提供会话上下文。
    - **生产级恢复机制**：`RDB/AOF重写 混合持久化` + `Docker数据卷挂载` + `PostgreSQL全量对话数据存储`，保证系统异常重启后短期记忆不丢失。
...
| **嵌入模型** | nomic-embed-text (本地) | 参考MTEB榜单评分，开源轻量嵌入模型的性能标杆。统一用于知识库向量化、用户查询向量化及长期记忆向量化，确保语义检索一致性 |
